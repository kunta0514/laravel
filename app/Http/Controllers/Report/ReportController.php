...
//        $type = 'week';
        ...
        $task_details = Task::where('actual_finish_date','>',$query_begin)
            ...
            ->get();
        $query_str="SELECT b.name,sum(CASE `status` WHEN 0 THEN 1 ELSE 0 END ) AS todo,sum( CASE `status` WHEN 1 THEN 1 ELSE 0 END ) AS doing,sum( CASE `status` WHEN 1 THEN 2 ELSE 0 END ) AS testing,
...
//       dd($query_str);die;
        ...
        $query_str="select NAME,sum(todo_workload) todo_workload,sum(deving_workload) deving_workload,sum(testing_workload) testing_workload,sum(dong_workload) dong_workload,role,sum(sum_workload) sum_workload from
             (SELECT b. NAME,IFNULL(case `status`  when 0 then sum(developer_workload) END,0) todo_workload,IFNULL(case `status`  when 1 then sum(developer_workload) END,0) deving_workload,0 as testing_workload,
